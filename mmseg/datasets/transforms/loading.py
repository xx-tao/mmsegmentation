...
@TRANSFORMS.register_module()
class LoadBinaryAnnotations(MMCV_LoadAnnotations):
    """Load annotations for semantic segmentation provided by dataset.
    ...
    def _load_seg_map(self, results: dict) -> None:
        ...
        Returns:
            dict: The dict contains loaded semantic segmentation annotations.
        """
        if Path(results['seg_map_path']).exists():
            img_bytes = fileio.get(
                results['seg_map_path'], backend_args=self.backend_args)
            gt_semantic_seg = mmcv.imfrombytes(
            ...
            if np.unique(gt_semantic_seg).size <= 2:
                gt_semantic_seg[gt_semantic_seg != 0] = 1
            else:
                raise ValueError(
                    'The binary mask should have only two unique values.')
        else:
            gt_semantic_seg = np.zeros(results['img'].shape[:2])
        # reduce zero_label
        if self.reduce_zero_label is None:
            self.reduce_zero_label = results['reduce_zero_label']
        assert self.reduce_zero_label == results['reduce_zero_label'], \
